...
As fixtures utilizam `yield` para separar a preparação e a limpeza dos dados. Os registros são criados antes da execução dos testes e excluídos ao final, evitando o acúmulo de usuários e produtos no ambiente público.

O usuário administrador e seu token utilizam `scope="session"`. Dessa forma, um único administrador é criado no início de cada execução do Pytest e seu token é compartilhado entre os testes que precisam de autenticação administrativa.

Ao executar apenas um teste isolado, as fixtures necessárias também são criadas normalmente para aquela execução. Ao final da sessão, o usuário administrador é excluído pela etapa de limpeza da fixture.

Essa estratégia reduz a quantidade de cadastros e logins realizados durante a suíte, sem depender de usuários ou tokens criados em execuções anteriores.
...
* Durante as primeiras execuções completas, um novo usuário administrador e um novo token eram criados em diferentes partes da suíte. Isso aumentava a quantidade de requisições realizadas contra o ambiente público e tornava a execução mais suscetível a respostas como `503 Service Unavailable`.

* Em outra etapa da investigação, foram observadas situações em que usuários recém-criados deixavam de ser encontrados e tokens recém-gerados deixavam de ser reconhecidos durante a mesma execução. Como a instância pública da ServeRest é compartilhada e seus dados podem ser alterados ou removidos, a suíte não deve depender de registros criados em execuções anteriores.

* Para reduzir requisições e manter os dados de autenticação restritos à execução atual, as fixtures do usuário administrador e do token foram configuradas com `scope="session"`. Assim, um novo administrador e um novo token são criados no início de cada sessão do Pytest, utilizados pelos testes que precisam deles e descartados ao final.

* Depois desse ajuste, os arquivos de login, usuários e produtos foram executados separadamente e, em seguida, a suíte completa foi executada com sucesso, totalizando `23 passed`.
